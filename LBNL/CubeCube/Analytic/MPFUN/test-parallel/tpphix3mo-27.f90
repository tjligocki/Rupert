...
!   This version uses the separate module "mpmodulem" for intermediate precision.
!   This version also includes OpenMP directives and wall-clock timing.
!   See OMP efficiency notes below by searching for #OMP.

!   David H Bailey   14 May 2016
...
parameter (idb = 2, ipal = 0, lcx = 128, lmx = 131072, mpi = 8, rb = 100.d0)
integer i, iq, i1, i2, j, j1, k, kd, kp, kq, l1, m, n, n1, &
  ndp, ndpm, nep, nepm, nwds, nwdsm
parameter (kd = 27, kp = 1, kq = 1, n = 163, ndpm = 810, ndp = 13000, &
  nep = 30 - ndp, nwds = int (ndp / mpdpw + 2), nepm = 20 - ndpm, &
  nwdsm = int (ndpm / mpdpw + 2))
double precision d1, d2, second, secondwc, secondwe, tm, tm0, tm1, &
  tm2, tm3, tmw0, tmw1, tmw2, tmw3
external second, secondwc, secondwe
...
!   Check to see if precision levels are high enough.

if (ndp > mpipl .or. ndpm > mpiplm) then
  write (6, 1) ndp, ndpm
1 format ('Increase precision levels in modules MPFUNF and MPFUNFM to at least'/ &
    2i8,' digits.')
  stop
endif
...
write (6, '("OMP max threads =",i4)') omp_get_max_threads ()

pi = mppi (nwds)
eps = mpreald (10.d0, nwds) ** nep
epsm = mprealmd (10.d0, nwdsm) ** nepm
...
tm1 = second ()
tmw1 = secondwc ()
write (6, 4) tm1 - tm0, secondwe (tmw1 - tmw0)
4 format ('Alpha CPU time =',f12.2/'Wall clock time =',f12.2/'Alpha =')
...
tm3 = second ()
tmw3 = secondwc ()
write (6, 5) tm3 - tm2, secondwe (tmw3 - tmw2), tm3 - tm0, secondwe (tmw3 - tmw0)
5 format ('PSLQM3 CPU time =',f12.2/'PSLQM3 wall clock time =',f12.2/ &
  'Total CPU time =',f12.2/'Total wall clock time =',f12.2)
...
t0 = mpcmplx (dcmplx (0.d0, 0.d0), nwds)
r1 = sqrt (sqrt (qq))
ds = -1.d0
...
theta3 = dcmplx (1.d0, 0.d0) + 2.d0 * t0
return
end
...
theta4 = dcmplx (1.d0, 0.d0) + 2.d0 * t0
return
end
...
!     epsm  mp_realm Tolerance for intermediate precision detection. This is
...
double precision d1, d2, d3, d4, deps, dplog10, dplog10m, rb, secondwc, &
  secondwe, tm0, tm1, times(6)
...
type (mp_realm) bound, dynrange, dynrangem, epsm, wa(n,n), wb(n,n), wh(n,n), &
  wsyq(n,nsq), wy(n), wn, w1, w2
external bound, dplog10, dplog10m, dynrange, dynrangem, secondwc, secondwe
...
tm1 = secondwc ()
times(2) = times(2) + secondwe (tm1 - tm0)
...
if (w1 < mprealmd (deps, nwdsm)) then
  goto 130
endif
...
tm1 = secondwc ()
times(3) = times(3) + secondwe (tm1 - tm0)
...
9 format ('Iteration',i8,3x,'MPM update')
  ...
  tm1 = secondwc ()
  times(4) = times(4) + secondwe (tm1 - tm0)
...
10  format ('Iteration',i8,3x,'MP update')
    ...
    tm1 = secondwc ()
    times(5) = times(5) + secondwe (tm1 - tm0)
...
tm1 = secondwc ()
times(6) = times(6) + secondwe (tm1 - tm0)
...
22 format ('Wall clock run times:'/(6f12.2))
...
t1 = mprealmd (1.d300, nwdsm)
t2 = mprealmd (0.d0, nwdsm)
...
integer i, n, n1, nwdsm
double precision d1
type (mp_realm) dynrangem, wy(n), t1, t2, t3
...
do j = 1, n
  do i = 1, n
    wa(i,j) = mprealmd (0.d0, nwdsm)
    wb(i,j) = mprealmd (0.d0, nwdsm)
  enddo

  wa(j,j) = mprealmd (1.d0, nwdsm)
  wb(j,j) = mprealmd (1.d0, nwdsm)
enddo
...
!   #OMP note: Parallelization here is often not worth the overhead, so some
!   experimentation may be required. To disable, use !! instead of !.

do i = 2, n
!!$omp parallel do private (ij,j,k)
  do j = 1, n - i + 1
    ij = i + j - 1
...
tmx1 = 1.d0 / epsm
tmx2 = mprealmd (2.d0, nwdsm) ** (nwdsm * mpnbt)
izmm = 0
mpr = nint (0.4d0 * n)
gam = sqrt (mprealmd (4.d0, nwdsm) / mprealmd (3.d0, nwdsm))
...
!   #OMP note: Parallelization here may not be worth the overhead, so some
!   experimentation may be required. To disable, use !! instead of !.

do i = 2, n
!$omp parallel do private (ij,j,k) schedule (dynamic, 1)
  do j = 1, n - i + 1
    ij = i + j - 1
...
zero = mprealmd (0.d0, nwdsm)
one = mprealmd (1.d0, nwdsm)
lup = min (m,n)
...
!   #OMP note: Parallelization here is usually cost-effective, but some
!   experimentation may be required. To disable, use !! instead of !.

!$omp parallel do private (i,j,k) schedule (static, 1)
do j = 1, n2
  do i = 1, n1
    c(i,j) = mprealmd (0.d0, nwdsm)
...
write (6, *) 'mxmm: n1, n2 =', n1, n2

!   #OMP note: Parallelization here is almost always cost-effective, but some
!   experimentation may be required. To disable, use !! instead of !.
