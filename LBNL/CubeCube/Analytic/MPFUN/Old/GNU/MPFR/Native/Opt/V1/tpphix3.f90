...
!     kd    Denominator of arguments of phi_2; default = 18.
!     kp    Numerator of first argument; default = 1.
!     kq    Numerator of second argument; default = 1.

!   PSLQ parameters:
!     idb   Debug level (0 - 3); default = 2.
!     n     Integer relation vector length = 1 + polynomial degree; default = 37.
!     ndp   Full precision level in digits; default = 700.
...
!     nepm  Log10 of intermediate precision epsilon, typically 20 - ndpm.
...
implicit none
double precision rb
integer idb, lcx, lmx, mpi, nsq
parameter (idb = 2, lcx = 64, lmx = 131072, mpi = 8, rb = 100.d0)
integer i, iq, i1, i2, j, j1, k, kd, kp, kq, l1, m, n, &
  ndp, ndpm, nep, nepm, nwds, nwdsm
parameter (kd = 18, kp = 1, kq = 1, n = 37, ndpm = 160, ndp = 700, &
  nep = 30 - ndp, nwds = int (ndp / mpdpw + 2), nepm = 20 - ndpm, &
  nwdsm = int (ndpm / mpdpw + 2))
double precision d1, d2, second, tm, tm0, tm1, tm2, tm3
external second
type (mp_real) al, eps, pi, qq, xx, yy
type (mp_real) epsm
...
type (mp_real) r(n), x(n)
save

write (6, 1) kd, kp, kq, mpi, n, ndp, ndpm, nep, nepm
1 format ('Poisson Phi_2(kp/kd,kq/kd) computation and analysis:'/ &
  'kd =',i4,'; kp =',i4,'; kq =',i4,'; mpi =',i4,'; n =',i4/ &
...
tm1 = second ()
write (6, *) 'cpu time =', tm1 - tm0
call mpwrite (6, ndp+20, ndp, al)

!   Attempt to recognize the result as an algebraic number.
...
tm3 = second ()
write (6, *) 'pslqm3 time =', tm3 - tm2
write (6, *) 'total cpu time =', tm3 - tm0
...
120   continue
      ...
      if (l1 + 100 > lmx) then
        write (6, *) 'line1 too long', l1 + k, lmx
        stop
      endif
      line1(l1+1:l1+1) = ' '
      l1 = l1 + 1
      ...
      l1 = l1 + k
      line1(l1+1:l1+lnm(i)+1) = '*' // nam(i)(1:lnm(i))
      l1 = l1 + lnm(i) + 1
    endif
  enddo
  ...
  i1 = 1
  write (6, 5)
5 format ('Output polynomial in Mathematica notation:')
