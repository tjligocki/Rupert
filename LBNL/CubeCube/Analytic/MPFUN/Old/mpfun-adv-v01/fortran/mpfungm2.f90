...
!  Language interface module for the *intermediate* precision (module MPFUNGM)
!  Variant 2: Precision level specifications are *required*; no real*16 support.
!  Search for !> for variant differences.

!  Revision date:  5 May 2016

!  This is essentially the same as MPFUNG (in file mpfung2.f90), except:
!    1. The module name is mpfungm; it references mpfunfm below.
!    2. The types mp_real and mp_complex datatypes are mp_realm and mp_complexm.
!    3. The constants mpwds, mpwdbt and mpwds6 are mpwdsm, mpwdsmbt and mpwdsm6.
!    4. The functions mpreal, mpreald, mpcmplx and mpcmplxdc are mprealm, mprealmd,
!       mpcmplxm and mpcmplxmdc.
!    5. The functions mpegamma, mplog2 and mppi are mpegammam, mplog2m and mppim.
!    6. The conversion function mpreal is extended to include mp_realm to mp_real,
!       with the new function mp_mtor; the function mp_realm is extended to
!       include mp_real to mp_realm conversion, with the new function mp_rtom.
!    7. The functions mp_setwprr, mp_fixlocrr, and mp_initvrr have been added
!       to support the two new conversion functions in the previous item.
!    8. The subroutine mpinit is mpinitm (although it is not implemented).

!  This module enables full functionality for the mp_realm and mp_complexm
!  datatypes, equivalent to that of mp_real and mp_complex.  This includes
!  all the usual mixed-mode combinations with double precision and double complex.
!  However, for the time being, mixed-mode operations between the mp_real and
!  mp_realm datatypes, and between the mp_complex and mp_complexm datatypes, are
!  NOT supported.  One must use the two conversion functions mpreal and mprealm
!  (see item 6 above).
...
!  DESCRIPTION OF THIS MODULE (MPFUNGM):
...
!   The mp_realm and mp_complexm datatypes are defined here:
...
!  These are for the mpreal and mprealm definitons.

private &
  mp_fixlocrr, mp_initvrr, mp_setwprr, mp_mtor, mp_rtom
...
!>  In variant #1, the next three module precedure lines are uncommented;
...
1       format ( &
        ...
        'Increase default precision in module MPFUNFM.')
      call mp_abrt (98)
    endif
    mp_setwp = iprec
  end function
...
!  This routine outputs an error message if iprec exceeds mpwds.
  ...
  function mp_setwprr (iprec)
    integer mp_setwprr
    integer, intent (in):: iprec
    if (iprec > mpwds) then
      write (mpldb, 1)
1       format ( &
        '*** MP_SETWPRR: requested precision level exceeds default precision.'/ &
...
1     format ('*** MP_CHECKDP: DP value has more than 40 significant bits:', &
      ...
      'Fix the issue, or else use functions mpprodd, mpquotd, mprealmd or mpcmplxmdc.'/ &
...
!  Multiplication routines:
  ...
  function mp_mulzd (za, db)
    ...
    call mpfrmuld (mp_mulzd%mpc(l3+1), za%mpc(l1+1), db, mprnd)
    return
  end function
...
!  Division routines:
  ...
  function mp_divzdc (za, dcb)
    ...
    call mpfrmuld (r1%mpr(1), za%mpc(l1+1), dble (dcb), mprnd)
    call mpfrmuld (r2%mpr(1), za%mpc(1), aimag (dcb), mprnd)
    call mpfr_sub (mp_divzdc%mpc(l3+1), r2%mpr(1), r1%mpr(1), %val(mprnd))
...
!  Value is negative and exponent is negative or zero.
  ...
  function mp_gammainc (ra, rb)
    implicit none
    type (mp_realm):: mp_gammainc
    type (mp_realm), intent (in):: ra, rb
    integer mpnw
    mpnw = max (int (ra%mpr(1)), int (rb%mpr(1)))
    mpnw = min (mpnw, mpwdsm)
    mp_gammainc%mpr(0) = mpwdsm6
...
!>  Otherwise in variant #2, uncomment these two lines:
  ...
  function mp_max (ra, rb, rc)
    ...
    type (mp_realm), intent (in):: ra, rb
    type (mp_realm), optional, intent (in):: rc
    integer mpnwbt
    call mp_fixlocr (ra)
    call mp_fixlocr (rb)
    mpnwbt = max (ra%mpr(1), rb%mpr(1))
...
!>  Otherwise in variant #2, uncomment these two lines:
    ...
    call mp_fixlocr (ra)
    call mp_initvrr (mp_mtor, mpnwbt)
    call mpfr_set (mp_mtor%mpr(1), ra%mpr(1), %val(mprnd))
    return
  end function
...
!>  Otherwise in variant #2, uncomment these two lines:
    ...
    call mp_fixlocrr (ra)
    call mp_initvr (mp_rtom, mpnwbt)
    call mpfr_set (mp_rtom%mpr(1), ra%mpr(1), %val(mprnd))
    return
  end function
...
!   Return working precision level (in words) of input MP value.
  ...
  function mp_wprecz (za)
    ...
    mp_wprecz = min (mpnwbt, mpwdsmbt) / mpnbt
    return
  end function
...
!   in the system parameters at the start of module MPFUNA.
  ...
  subroutine mp_outz (iu, ln, nd, a)
    ...
    type (mp_complexm), intent (in):: a    
    call mp_outr (iu, ln, nd, mp_ztor (a))
