...
To avoid numerous factors of 1/2 we are going to solve the equivalent problem
where the $n$-cube has vertices with coordinates $\pm1$ and we look at the
{\it ratio} of the maximal $m$-cube side length to the $n$-cube side length
...
Using the facts that the edge vectors have equal length, $l = 2 f(3,4)$, and
are mutually orthogonal, we find that $b = -a$ and obtain the following
system of equations:
\begin{align*}
2a^2       + c^2         &= l^2 = 4 f(3,4)^2 \\
2ad        - c(c+2)      &= 0                \\
2d^2       + (c+2)^2 + 4 &= l^2 = 4 f(3,4)^2 \\
2(a-d-2)^2               &= l^2 = 4 f(3,4)^2 
\end{align*}
Letting $t = f(3,4)^2$, these equations can be reduced to:
\begin{equation*}
...
\end{align*}
and this means $f(3,4) \ge 1.0074347568...$
