...
There are difficulties in jamming objects into confined spaces.  Objects of similar, and even identical, size and shape can induce frustration, and corners in spaces can be inaccessible (while at the same time providing novel constraints).  Optimal packings of nonoverlapping objects have been puzzled over for thousands of years, and centuries have passed without substantial progress on certain problems. For example, the densest arrangement of equal-sized regular tetrahedra in 3-space is still unknown [Torquata/Conway, Glotzer, Cohn]. The subjects of covering, filling and jamming have emerged from attempts to reconcile geometrical constraints and energy minimization. In recent years, theoretical ideas and computational techniques from physics, such as simulated annealing, parallel tempering, molecular dynamics\cite{glotz1,glotz2}, and models of quasicrystalline order\cite{??kohn1}, have re-energized some of these age-old problems. Here, in this Letter, we take a physics approach to a problem that stems from the 17th-century exploits of the European prince, Rupert of the Rhine, namely that of finding the largest square that fits entirely inside a unit cube \cite{rupert-hist1}.
...
\noindent {\bf Rupert's problem: History.}
In the mid-17th century, Prince Rupert (1619-1682), also called Ruprecht, and someone known for both military and scientific inventions, posed the following geometrical puzzle as a wager: Can one form a hole in a cube such that a second cube of the same size may pass through the hole?
This was the original Rupert problem, and John Wallis showed the answer to be ``yes''.  Wallis [wallis], centering a square hole on the plane normal to the cube's main diagonal, found that this square could have sidelength as large as $\sqrt{6}-\sqrt{2}=1.03527...$.  Wallis also thought this solution was maximal; on this count, he was mistaken.  The optimal square hole -- equivalent to the largest square in a unit cube -- lies on a plane with lesser symmetry, oblique to the main diagonal, as discovered by Pieter Nieuwland, in the late 18th-century.  He calculated a maximal sidelength of $\sqrt{9/8} = 1.06066...$ (Fig.~\ref{fig:fig1}a). Centuries later, Gardner, in a 1966 column devoted to hypercubes [gardner-sciam], asked for the sidelength of the largest 3-cube inscribable in a unit 4-cube. Gardner's article elicited a number of correct solutions: the sidelength $f(3,4)$ of the 3-cube in the 4-cube is approximately $1.00743...$ (E. I. Bosch first discovered this value, but it has been called ``DeVicci's constant" \cite{finch} because DeVicci was the first to provide a proof of its correctness\cite{devicci-shultz}.)

\vspace{.15in}

\noindent {\bf Generalized Rupert problem.}
It is but a short step from Gardner's question to ask for the sidelength, $f(n-1,n)$, of the largest $(n-1)$-cube inscribable in an $n$-cube.  The resulting numerics are so striking that we display them here for $n<16$, even before discussing our methods for computing them:
...
\begin{equation*}
  ...
  \end{matrix}
\end{equation*}
Although the Babylonians had the solution $f(1,2)=\sqrt{2}$\cite{neugebauer},
it is still worth discussing for three reasons: i) features of this geometry
have similarities to the general problem, ii) a bound on $f(n-1,n)$ can lead
to a bound on $f(n,n+1)$, and iii) a solution to $f(n-1,n)$ can be perturbed
to build a solution to $f(n,n+1)$.

By examining this family of numerical lower bounds, one can see the asymptotic
law 
%%%$f(n-1,n) \sim 1+\frac{1}{2} 9^{2-n}$ [need to fix this, if it is not quite correct]
$f(n-1,n) \sim 1+\kappa/9^{n}$.
We'll see that asymptotics of this type arise in other families as well.

It is but another short step to consider the {\it generalized Rupert problem}
for the pair of dimensions $(m,n)$: What is $f(m,n)$? Where $f(m,n)$ is the
sidelength of the largest $m$-cube that fits inside a unit $n$-cube.
...
discussing a matrix, we'll indicate if it is a V, E or R matrix. (When writing the matrix elements, so as to avoid numerous factors of 1/2, we will always consider the $n$-cube to have vertex coordinates $\pm1$, {\it i.e.} to have sidelength $2$.)
...
a        &  0 & b        &  c   \\
d        & -2 & a+b-d    & -c-2 \\
2a+b-d-2 &  0 & 2a+b-d-2 &  0   \\
\end{bmatrix*}
\end{align*}
where $a = -x+1$, $b = -y+z$, $c = -w-1$, and $d = -x+z$.
Using the facts that the edge vectors have equal length, $2 f$, and
are mutually orthogonal, we find that $b = -a$ and obtain the following
system of equations:
\begin{align*}
2a^2       + c^2         &= 4 f^2 \\
2ad        - c(c+2)      &= 0                \\
...
2(a-d-2)^2               &= 4 f^2 
\end{align*}
Letting $t = f^2 = f(3,4)^2$, these equations can be reduced to:
\begin{equation*}
p(t) = 4 t^4 - 28 t^3 - 7 t^2 + 16 t + 16 = 0
\end{equation*}
...
c &= 161             + (184841 - 4320\sqrt{1290})^{1/3} \\
  &  \hspace{0.39in} + (184841 + 4320\sqrt{1290})^{1/3}.
\end{align*}
This provides the lower bound $f(3,4) \ge  \sqrt{t} = 1.0074347568...$. DeVicci was the first to provide a case-by-case proof\cite{devicci-shultz} that this bound is optimal. This approach to finding bounds for
$f(m,n)$ and proving they are maximal is problematic, and we adopt a different theoretical and numerical approach to exploring $f(m,n)$.
...
Another way to view the problem is this: given the axis-aligned $n$-cube $[-1,1]^{n}$ and an axis-aligned 
$m$-cube $[-1,1]^{m}$ in $n$-dimensions (with all vertex coordinates above $m$ set equal
to zero), what is the rotation matrix and scaling that takes the canonical
$m$-cube into an optimal m-cube?
It is enough to consider the rotation of the set of $2^{m}$ vertex vectors, since the $m$-cube is the convex hull of this set. The rotation matrix, $R$, is
simply an element of $SO(n)$ and the scale factor $f$ is a real scalar.  Once you
know $R$, $f$ can easily be computed, thus only $R$ needs to be found.

$R$ is an $n \times n$ orthonormal matrix with entries $r_{ij}$. The generalized
Rupert problem then becomes a min-max problem defined on the matrix representation of $SO(n)$:
\begin{equation*}
f(m,n) = \left(
           \min_{R \in SO(n)} \left(
             \max_{i=1}^{n} \sum_{j=1}^{m} |r_{ij}|
           \right)
         \right)^{-1} .
\end{equation*}

The min-max formula Eqn(xx) obtains because $\sum_{j=1}^{m} |r_{ij}|$ is the maximal extent of the $2^m$ vertex vectors in the direction $i$ -- the absolute value is needed because of all the $2^m$ assignments of $\pm 1$. 
The smallest canonical ({\it i.e.} axis-aligned) $n$-cube containing the $m$-cube si the maximum over the $n$ directions, and by minimizing the max, one obtains $1/f$, the inverse of the scale factor.
...
%%%Transition into the numerical techniques we'd used to explore the overall problem. Metropolis/simulated annealing.
...
\hline
n &   f(n-1,n)-1   & f(G(n-1),G(n)))-1 & f(F(n-1),F(n)) \\
\hline
2 & 1.414213562373 &   1.732050807568  & 1.414213562373 \\
\hline
3 & 1.060660171779 &   1.096376317177  & 1.060660171779 \\
\hline
4 & 1.007434756884 &   1.012102089880  & 1.096376317177 \\
\hline
5 & 1.000839446859 &   1.001375497904  & 1.066018271667 \\
\hline
6 & 1.000093473576 &   1.000153317012  & 1.069164803583 \\
\hline
7 & 1.000010388837 &   1.000017042268  & 1.066490851570 \\
\hline
8 & 1.000001154355 &   1.000001893685  & 1.066768286664 \\
\hline
9 & 1.000000128262 &   1.000000210410  & N/A            \\
\hline
\end{tabular}
\end{center}

Where $G(n) = 2n-1$ and $F(n)$ the $n$th Fibonacci number.

\vspace{0.15in}

\noindent{\bf Ten Families Illustrated in Five Dimensions.} 
After amassing a substantial amount of empirical data the first issue that
arises is how to display and understand the data.  The data naturally lives on
a lattice defined by the natural numbers $n$ and $m$ where $m \le n$.

If the conjecture that $f(am,an) = f(m,n)$ (see below) is true, then $f$ is
constant when the ration $m/n$ is fixed.  Thus, all the results can be
projected onto the interval $0 \le m/n \le 1$ and it is convenient to plot
$1/f^2$ on the vertically.  This results in Fig.~\ref{fig:fig3}a.

Noting the similarity in this graph between the intervals $[1/(k+1),1/k]$
where $k$ is a natural number, Fig.~\ref{fig:fig3}b shows these interval
rescaled and offset in both directions to lie on top of each other.

Before doing extensive computations the values of a few families of $f$ can be
proven: $f(1,n) = f(2,2n) = \sqrt{n}$, $f(2,2n+1) = \sqrt{n+1/8}$.  Based on
some observations and extensive computations additional conjectures can be
made about the values of other families of $f$:  $f(am,an) = f(m,n)$,
$f(m,2m-1) = \sqrt(2) (1+\sqrt(2m))/(2+\sqrt(2m))$.

Looking at the form of the optimal rotation matrix obtained via simulated
annealing, allowed the use of Mathematica to get very precise values for
$f(n-1,n)$ and $f(n-2,n)$ using a contraction mapping to get a maximum.

Finally, looking at the values of $f$ obtained via simulated annealing other
families were suggested:  For each $m$ and $0 \le c < m$, $f(m,am+c)$ form a
family as $a$ varies over the natural numbers.

To illustrate these families look at a variety of values for $m$.  For $m = 1$,
this is part of the $f(1,n)$ family.  For $m = 2$, there are two families, $n$
mod $2 = 0$ and $n$ mod $2 = 1$.  The first coincides with $f(1,n)$ and the
other is a new family.
...
``golden" or Fibonacci family:  f(1,2), f(2,3), f(3,5), f(5,8), etc.  These values should
