...
!  Language interface module (module MPFUNH)
...
private &
  ...
  mp_bessel_jn, mp_bessel_k, mp_bessel_kn, mp_bessel_y, mp_bessel_y0, &
  mp_bessel_y1, mp_bessel_yn, mp_binmd, mp_checkdp, mp_checkqp, &
  mp_ccos, mp_cexp, mp_clog, mp_conjg, mp_cos, mp_cosh, mp_csin, &
  mp_csqrt, mp_cssh, mp_cssn, mp_dctoz, mp_dctoz2, mp_decmd, &
  mp_digamma, mp_digamma_be, mp_dtor, mp_dtor2, mp_eform, mp_egamma, &
  mp_erf, mp_erfc, mp_exp, mp_expint, mp_fixlocr, mp_fixlocrm, &
  mp_fixlocz, mp_fixloczm, mp_fform, mp_gamma, mp_hurwitz_zetan, &
  mp_hurwitz_zetan_be, mp_hypergeom_pfq, mp_hypot, mp_incgamma, &
  mp_initvr, mp_initvz, mp_inpr, mp_inpz, mp_log, mp_log10, &
  mp_log_gamma, mp_log2, mp_max, mp_min, mp_mod, mp_nrt, mp_outr, &
  ...
  mp_readz4, mp_readz5, mp_rtod, mp_rtoq, mp_rtom, mp_rtor, mp_rtoz, &
  mp_setwp, mp_sign, mp_sin, mp_sinh, mp_sqrt, mp_struve_hn, mp_tan, &
  mp_tanh, mp_wprec, mp_wprecz, mp_writer, mp_writez, mp_zeta, &
  mp_zeta_int, mp_zeta_be, mp_ztodc, mp_ztor, mp_ztor2, mp_ztoz, &
  mp_ztozm
...
120 continue
  ...
  function mp_expzz (za, zb)
    ...
    type (mp_complexm), intent (in):: za, zb
    type (mp_realm):: r1, r2, r3, r4, r5, r6
    integer l1, l2, l3, mpnwbt
    call mp_fixloczm (za)
    call mp_fixloczm (zb)
    l1 = za%mpc(0)
...
!>>
  ...
  function mp_bessel_i (ra, rb)
    ...
    call mp_fixlocrm (ra)
    call mp_fixlocrm (rb)
    mpnwbt = min (ra%mpr(1), rb%mpr(1), mpwdsmbt)
    mpnw = mpnwbt / mpnbt
...
!  Value is negative and exponent is negative or zero.
  ...
  function mp_hypergeom_pfq (np, nq, aa, bb, xx)
    ...
    call mp_fixlocrm (xx)
    mpnwbt = min (xx%mpr(1), mpwdsmbt)
    mpnw = mpnwbt / mpnbt
    call mp_initvr (mp_hypergeom_pfq, mpnwbt)
    call mphypergeompfq (np, nq, mpwdsm, aa(1)%mpr(0:), bb(1)%mpr(0:), &
...
1     format ('*** MP_PI: Pi must be precomputed to precision',i9,' words'/ &
  ...
  subroutine mp_polylog_ini (nb, rb, iprec)
    implicit none
    integer, intent (in):: nb
    type (mp_realm), intent (out):: rb(abs(nb))
...
!>>
  ...
  function mp_polylog_neg (nb, rb, rc)
    implicit none
    integer, intent (in):: nb
    type (mp_realm):: mp_polylog_neg
    type (mp_realm), intent (in):: rb(abs(nb)), rc
    integer n1, mpnw, mpnwbt
    mpnwbt = max (int (rb(1)%mpr(1)), int (rc%mpr(1)))
...
!>>
    ...
    call mpfrset (mp_rtom%mpr(1:), ra%mpr(1:), mprnd)
    return
  end function
...
!>>
    ...
    call mp_fixlocz (za)
    l2 = mpwdsm6
    call mp_initvz (mp_ztozm, mpnwbt)
    call mpfrset (mp_ztozm%mpc(1:), za%mpc(1:), mprnd)
    call mpfrset (mp_ztozm%mpc(l2+1:), za%mpc(l1+1:), mprnd)
    return
  end function
